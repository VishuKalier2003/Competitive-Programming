...
public class MinimizeCoins {
    ...
    public static int solve(final int n, int money, final int nums[]) {
        ...
        for(int i = 1; i <= money; i++) {
            for(int num : nums)
                // Check the sentinel Integer.MAX_VALUE so that you don't get -1 when adding 1 to Integer.MAX_VALUE
                if(i-num >= 0 && dp[i-num] != Integer.MAX_VALUE)
                    dp[i] = Math.min(dp[i], 1 + dp[i-num]);
        }
        // Imp- Since the range is till 1_000_000 so the ways can be impossible only when the count is more than this
