...
public class MoneySums {
    ...
    public static StringBuilder solve(final int n, final int sum, final int nums[]) {
        ...
        // Imp- We perform operations as reverse, for addition we do subtraction from right to left and for multiplication we do division
        for(int num : nums)     // Imp- O(n)
            // We make a range for each number from sum to num, and check if state-num is true, if yes, then it implies that state-num is already valid and adding num to that state will make the state also valid
