...
public class P8MaximumManhattan {
    ...
    public static void solve(final int n, final Point points[]) {
        long vals[] = new long[4], maxVal[] = new long[4], minVal[] = new long[4];
        ...
        for(Point p : points) {
            ...
            long best = 0L;
            for(int i = 0; i < 4; i++) {
                maxVal[i] = Math.max(maxVal[i], vals[i]);
                minVal[i] = Math.min(minVal[i], vals[i]);
                best = Math.max(best, maxVal[i] - minVal[i]);
            }
