...
public class ColorTree {
    ...
    public static void main(String args[]) {
        ...
        // IMP- For a tree that can have n children, use Map with Set to identify and extract the data real quick
    ...
    public static int coloringTime(final int n, Map<Integer, Set<Integer>> tree, int color[]) {
        int colorCount = 0;
        for(int i = 1; i <= n; i++)
            // IMP For every node starting from 1, count the adjacent nodes colored differently, that means we need to color them again, and they do not follow the parent color
            colorCount += childrenColor(i, color[i], tree, color);
        return colorCount;
    }

    public static int childrenColor(final int source, final int sourceColor, Map<Integer, Set<Integer>> tree, int color[]) {
        int count = 0;
