...
public class LRS {      // IMP- Longest Repeating Subsequence
    public static void main(String[] args) {
        try (Scanner sc = new Scanner(System.in)) {
            final String s1 = sc.next(), s2 = sc.next();
            final int n = s1.length(), m = s2.length();
            System.out.println("----- Pure Recursion -----");
            measurePerformance(() -> System.out.println("LRS Length (Recursion): " + helper(0, 0, n, m, s1, s2)));
            System.out.println("\n----- Memoization -----");
            ...
            for(int row[] : memo)
                Arrays.fill(row, -1);
            measurePerformance(() -> System.out.println("LRS Length (Memoization): " + memonize(0, 0, n, m, s1, s2, memo)));
            System.out.println("\n----- Tabulation -----");
            measurePerformance(() -> System.out.println("LRS Length (Tabulation): " + tabulate(n, m, s1, s2)));
        }
    }
    ...
    public static int helper(int i, int j, final int n, final int m, final String s1, final String s2) {
        if(i == n || j == m)        // If any of the string is completely traversed, we cannot increase the LCS further
            return 0;
        if(s1.charAt(i) == s2.charAt(j) && i != j)        // When characters match, increase LCS by 1
            // IMP- check extra condition of i != j
    ...
    public static int tabulate(final int n, final int m, final String s1, final String s2) {
        ...
        for(int i = 1; i <= n; i++)
            for(int j = 1; j <= m; j++) {
                ...
                if(s1.charAt(i-1) == s2.charAt(j-1) && i != j)      // IMP- extra condition of i != j
                    dp[i][j] = 1 + dp[i-1][j-1];
