...
public class RatingSystem {
    ...
    // Maximum Negative Sum subarray
    public static StringBuilder solve(final int n, final int[] nums) {
        long maxPrefix = 0L, sum = 0L, drop = 0L, k = 0L;
        for(int num : nums) {
            sum += num;
            maxPrefix = Math.max(maxPrefix, sum);
            if(sum - maxPrefix < drop) {
                // deeper drop found (more negative subarray)
                drop = sum - maxPrefix;
                k = maxPrefix;
            }
        }
        return new StringBuilder().append(k).append("\n");
    }
}
