...
public class RoundDance {
    ...
    public static StringBuilder solve(final int n) {
        ...
        int min = cycle + (bamboo > 0 ? 1 : 0);
        int max = cycle + bamboo;
        return new StringBuilder().append(min).append(" ").append(max).append("\n");
    }

    public static void dfs(int node, boolean vis[], int degree[]) {
        vis[node] = true;
        if(g.get(node).size() == 1)
            degree[0]++;
        for(int neighbor : g.get(node))
            if(!vis[neighbor])
                dfs(neighbor, vis, degree);
    }
}
