...
public class SegmentTreeLevel4 {
    ...
    public static void main(String[] args) {
        ...
        }, "Segment-Tree-(Enriched-Segment-Tree)", 1 << 26);
        t.start();
        try {
            t.join();
    ...
    public static class SegmentTree {
        ...
        private void build(int root, int l, int r, long nums[]) {
            if (r - l == 1) {
                sum[root] = nums[l];
                prefix[root] = suffix[root] = maxSum[root] = Math.max(0L, nums[l]);
                return;
            }
            int mid = (l + r) >>> 1;
        ...
        private void queryUpdate(int root, int l, int r, int qIdx, int value) {
            if (r - l == 1) {
                sum[root] = value;
                prefix[root] = suffix[root] = maxSum[root] = Math.max(0L, value);
                return;
            }
            int mid = (l + r) >>> 1;
            if (qIdx < mid)
    ...
    public static class Node {
        ...
        public Node() {
            this.sum = this.suffix = this.prefix = this.maxSum = 0L;
        }
