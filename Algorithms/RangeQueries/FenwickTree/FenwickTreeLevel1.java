...
public class FenwickTreeLevel1 {
    ...
    public static StringBuilder solve(final int n, final int q, final int nums[]) {
         ...
         * The tree will be of size N, query updates the prefix so when we push or
         * reorder, we will do -1 for prefix range(1, old)
         * and +1 for prefix range (1, new) and push the query pointer to left at each
         * query
         */
        ...
        for (int i = 0; i < q; i++) {
            int query = nums[i];
            // store the answer since I want to check the indices before (excluding current
            // element)
